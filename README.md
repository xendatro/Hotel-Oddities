...
- ReplicatedStorage\Services\ChaosWarningSoundService.luau — Plays hallway ambience and a 2D incoming sting for the warned hallway you are in, next to, or whose door you can see, with its own gain curve, an anchor held up-corridor at a fixed distance so camera rotation cannot swing it, a cut timed off the real Chaos model, the sting held to a genuine predicted approach inside a tight StingDistance so it cannot fire full-volume for a distant fly-past, and the normal ambience muted only once the bed is loud enough to replace it.
...
- ReplicatedStorage\Services\FirstPersonCameraService.luau — Walking camera bob, strafing tilt and custom cursor setup for first person.
...
- ReplicatedStorage\Services\MirrorRoomService.luau — Client tag listener that binds a MirrorRoom class to every `MirrorRoom` tagged connector.
...
- ReplicatedStorage\Services\PhotoCaptureService.luau — Client shutter for the tripod camera: authored flash, GUI hide, local ShadowFigure clone, lens-locked screenshot through CaptureGalleryService and restore.
- ReplicatedStorage\Services\PhotoDevelopService.luau — Binds the Studio-authored film preview and plays a finished capture with a mouse-free keep-or-burn prompt.
...
- ReplicatedStorage\Classes\MirrorRoom.luau — Renders upside-down reflections of every player and enemy standing in a mirrored connector room.
...
- ReplicatedStorage\Classes\Tools\Camcorder.luau — Client camcorder; stows and disables itself while the Studio-authored REC/STOP HUD runs, then offers keep or burn.
...
- ReplicatedStorage\Configs\MirrorRoomConfig.luau — Bounds padding, retry delay and the instance classes stripped from a mirror-room reflection.
