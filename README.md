...
- ReplicatedStorage\Services\GravityWarpService.luau — Client executor for the Gravity Warper; rolls the player and camera onto a ceiling-locked plane while keeping look input relative to the displayed screen, then rolls them back down.
...
- ReplicatedStorage\Classes\Wallstick\ — Vendored EgoMoose Rbx-Wallstick surface-sticking controller (modernized constraints), driven through WallstickService; self-contained — patches gravity and screen-relative look handling into the live camera and swaps animation/sound drivers only while enabled.
