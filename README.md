...
- ReplicatedStorage\Services\ChaosWarningSoundService.luau — Plays hallway ambience and a 2D incoming sting for the warned hallway you are in, next to (with a wall check) or whose door you can see, gained off the distance to where Chaos actually is rather than to the hallway, with an anchor that travels the corridor ahead of it, a cut timed off the real Chaos model, the sting held to a genuine predicted approach inside a tight StingDistance, and the normal ambience muted only once the bed is loud enough to replace it.
