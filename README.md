...
- ReplicatedStorage\Services\GravityWarpService.luau — Client executor for the Gravity Warper; rolls the player, camera and gravity-relative controls onto a ceiling-locked plane for the warp duration, then rolls them back down.
...
- ReplicatedStorage\Classes\Wallstick\ — Vendored EgoMoose Rbx-Wallstick surface-sticking controller (modernized constraints), driven through WallstickService; self-contained — patches and can snap the gravity-aware camera, and swaps animation/sound drivers only while enabled.
