...
- ReplicatedStorage\Services\ChaosWarningSoundService.luau — Plays hallway ambience and an incoming sting for the warned hallway you are in, next to (with a wall check) or whose door you can see, opening exactly when the red reaches your own position and cutting when Chaos passes, from a source on the hallway centre line that follows you along it so you cannot outrun it, with volume left entirely to the emitters' authored rolloff.
...
- ReplicatedStorage\Services\InterfaceService.luau — Main menu page switching, blur, FOV pull-back and mouse unlocking, including Q toggling through the mouse blocker.
...
- ReplicatedStorage\Classes\Tools\Camcorder.luau — Client camcorder; stows and suppresses its viewmodel while the Studio-authored REC/STOP HUD runs, reports recording completion, then offers keep or burn.
