...
Documentation\
  ...
  ReplicatedStorage\Services.md   Classes.md   Configs.md   Sounds.md   Tools.md   Modules.md   Frameworks.md
...
| `KitVisualService` / `KitCard` | `ReplicatedStorage\Services`, `ReplicatedStorage\Classes` | Kit tile dressing, rarity styling, headshot portraits with viewport fallback, and stat/item rows |
...
| `CoinService` | `ServerStorage\Services` | Coin balance: read, award with DoubleCoins, spend, refund and replicate, every change logged as an economy event |
| `GemService` | `ServerStorage\Services` | Gem balance: read, award with DoubleGems, spend, refund and replicate, every change logged as an economy event |
| `BadgeService` / `AchievementService` / `BadgeIconService` | `ServerStorage\Services`, `ReplicatedStorage\Services` | Awarding a badge id, the rules that decide when each badge is earned, and resolving a badge's icon image |
...
| `Icon` / `TopbarIconService` | `ReplicatedStorage\Classes`, `ReplicatedStorage\Services` | Roblox topbar icons (vendored TopbarPlus) and the pages they open |
...
- ReplicatedStorage\Services\CaptureGalleryService.luau — Takes, keeps, burns, session-hides and reloads the player's own screenshots and camcorder tapes through Roblox's Captures API, requesting device access only from explicit save/access actions.
...
- ReplicatedStorage\Services\MathService.luau — Shared pure-math helpers: easing (smoothstep, the named `Eases` table and a back-out ease), frame-rate independent lerp alphas, horizontal vector work, rotation vectors, angles, pulses and comma-grouped number formatting.
...
- ReplicatedStorage\Services\MirrorRoomService.luau — Shared mirror-room lookup, bounds and mirror-plane maths, plus the client tag listener that binds a MirrorRoom class to every `MirrorRoom` tagged connector.
...
- ReplicatedStorage\Services\NotificationService.luau — Client top-center notifications, each a torn paper strip in the game's own hand that drops in, holds and rises away.
...
- ReplicatedStorage\Services\SistersVertigoService.luau — The vertigo screen effect for locking eyes with the Sisters: cold flash, blur, colour drain, FOV pull, rippling rings, jolt shake and sting, released as the ceiling warp begins.
...
- ReplicatedStorage\Classes\CameraTrack.luau — Keyframed camera and grade spline evaluator: C1-continuous channels with anchors and named eases, handheld noise driven by time, and a spring blend out of the live camera.
...
- ReplicatedStorage\Classes\KeyPill.luau — Drives one authored key-prompt pill with the interaction prompt's look and motion: key label per input type, hold fill with a key tint, flash, dimming and a tap button.
...
- ReplicatedStorage\Classes\Icon\ — Vendored TopbarPlus v3.4.0 topbar icon library, driven through TopbarIconService.
...
- ReplicatedStorage\Configs\IntroCutsceneConfig.luau — The intro cutscene's version, seen and playing attributes, command, remotes, triggers, outcomes, length and streaming points (read by the server too), plus the client's gui names, keys, text, captions, sound cues, staging, light sweep, camera and grade keyframes and event timeline.
...
- ServerStorage\Services\AchievementService.luau — Every badge rule: awards the join, room, computer, escape and killed-by badges from the owning services' signals, catching returning players up on what they already earned.
...
- ServerStorage\Services\ComputerService.luau — Persists each player's hacked computer ids, restores them across sessions, resets them on demand, replicates streaming-safe color completion and exit eligibility, logs the funnel step and a `ComputerHacked` event per hack, and fires `Hacked` with the computer's chip colour.
...
- ServerStorage\Services\DataSaveService.luau — Loads, reconciles and releases per-player ProfileService profiles, including hacked computer ids, the escape count and the seen intro version, keyed separately for Studio sessions so playtests never contest the live game's session lock.
...
- ServerStorage\Services\FriendReviveService.luau — Paid "revive your friend" offers, friend checks and the product receipt that grants the revive, attributed to a friend in the `Revived` event.
...
- ServerStorage\Services\CoinService.luau — Coin balance service with centralised earning, spending, refunds and DoubleCoins handling, every change logged as a Roblox economy event.
- ServerStorage\Services\GemService.luau — Gem balance service with centralised earning, spending, refunds and DoubleGems handling, mirrored to an attribute, replicated to the client, and every change logged as a Roblox economy event.
...
- ServerStorage\Services\IntroCutsceneCommandService.luau — Admin `/intro` (alias `/cutscene`) that queues the intro and sends the caller on the full elevator ride, and `/intro reset [player]` that clears the seen flag.
...
- ServerStorage\Services\MirrorStalkerService.luau — Rolls and owns the mirror-room encounter, spawning one MirrorStalker behind a player who enters the room, whether or not others are inside.
...
- ServerStorage\Classes\Oddities\HallwayCrush.luau — Hallway oddity that closes both walls of one junction-free stretch of corridor inward until they seal, dragging the pilasters, lanterns, paintings and doors with them and crushing unprotected players whose HRP overlap the lethal volume past the configured threshold; wall layers are identified by their parent folder when the map carries no `WallLayer` attribute, every moving piece at a run end is carried out to the crossing corridor's own wall line so the sealed run fills the branch mouth flush instead of sitting at the back of an alcove, and each moving wallpaper piece is papered on its bare end face for as long as the effect runs.
...
- ServerStorage\Classes\Tools\Gravity Warper.luau — Server half of the gravity warper; checks and consumes one, then starts the warp through the shared server GravityWarpService.
