...
A single polling scheduler fires each light's red warning (`LightService:WarnRed`) and each hallway stretch's `ChaosWarning` oddity `WarningTime` (15s) before Chaos's timed arrival, holding any warning whose players are outside its cull range until they come near or it expires; living root positions are resolved once per poll rather than once per waiting warning. Spans are resolved with the route's own travel direction so a junction node telegraphs the hallway Chaos actually runs down.

A warned region is the **stretch of a span the route actually travels**, never the whole span. This distinction is the difference between a telegraph and a false alarm. `Hallways.StraightSpanAt` returns the entire merged run of collinear hallway parts through a point, so a main corridor comes back hundreds of studs long even where the route only clips one end of it before turning off. Warning on the whole span put every player standing anywhere in that corridor **inside** the region box — gain 1, the bed at full volume — while Chaos never came within a hundred-plus studs of them, and the arrivals were no better: they were taken by projecting the corridor's two far ends onto whichever route leg happened to be nearest, discarding the distance that projection returned, so ends the route never visits were assigned arrival times from unrelated legs. Lerping those between them handed the client a plausible-looking lead and fired the full-volume incoming sting for a Chaos that was about to run somewhere else entirely. It also disagreed with the red lights, which are placed off real proximity to the route (`LightRouteRange`) and so stayed dark.

The route is therefore walked once, accumulating cumulative travel, and each point's resolved span is grouped into contiguous runs by `spanKey`. Each run records the minimum and maximum distance along the span axis together with **the travel at those two extremes** — paired, not min/max travel independently, so a corridor entered from the far end still reports which of its ends Chaos reaches first, which is what `approachSign` reads on the client to place the anchor up-corridor. `trimSpan` cuts the span to that interval, padded by `WarnSpanPadding` and clamped to the real span, and the arrivals come straight from the paired travels. A run that merely crosses a corridor at an intersection now warns a short box at the crossing rather than the whole corridor. Warnings expire `LightPassDelay` after Chaos leaves the stretch rather than at the far end of the corridor, leaving the client just enough tail to run its pass hard-cut before the region stops.
...
The population manager: on a heartbeat tick it tops up resident enemy counts, schedules Chaos runs (retrying on the next tick rather than after a whole `ChaosInterval` when `ChaosService` reports the spawn was abandoned), rotates the Stalker across players, spawns Ghosts, and despawns expired enemies that are unengaged and out of sight; an enemy counts as engaged while it has a target, is mid peek sequence (`Peeking`), or is in a Chase, Attack, Stalk, Reveal, Investigate or Flee state. Placement scores candidate danger points or hallway-graph nodes by danger, player proximity and enemy spacing, and rejects anything visible from a player's eye. Defines no-op stubs for its whole API first and returns early unless `FLAGS.Enemies` and `FLAGS.Director`.
...
- API: `RoomService:GetDoorApproach(room: Model, agentRadius: number) -> (Vector3?, Vector3?, Vector3?)` — pathfinding standing point outside the door, the door point, and a contact point `DoorConfig.KnockDistance` studs off the door; the standing point clears the door by `agentRadius + DoorConfig.ApproachPadding` (at least `DoorConfig.MinApproachDistance`) so the walk up to it does not graze the lantern columns that flank every room door and protrude about half a stud into the corridor, and the contact point is the arm's-length spot a caller steps to directly once that walk finishes
