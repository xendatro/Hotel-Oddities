...
The full humanoid-enemy base: pathfinding with prefetch, direct-pursuit/lane-clearance movement, line-of-sight and observation checks, target acquisition, network-ownership management, and a library of shared state functions (Idle/Wander/Patrol/Attack/Retreat/Stunned/RoomReaction/Despawn). Danger-weighted hallway-graph patrol and safe-room reactions live here too. Attacks deal the config's `Damage` through `DeathService:Hit` rather than killing, and what happens after a landed hit is the config's `AfterAttack`: `Recover` (the default) keeps following the target at `ChaseSpeed * RecoverSpeedScale` for `RecoverTime` seconds and then chases again, so the player gets a window to break away; `Retreat` hands off to the shared Retreat state, which sprints at `RunAwaySpeed` to `HideSpotService` cover away from the victim until it is unobserved and `RunAwayDespawnDistance` from everyone (or `RunAwayTime` runs out) and then removes itself silently, no despawn particles, sound or fade, through `DespawnSilently`.
...
- API: `NPC:Attack(player: Player) -> boolean` — a damage hit through `DeathService:Hit`, false when refused
- API: `NPC:Recover(target: Player)` — the slowed follow used after a `Recover` hit
- API: `NPC:DespawnSilently()` — despawn with no effect, sound, fade or delay
- API: `NPC:NearestPlayerDistance() -> number`
...
- API: `NPC:MoveThrough(destination: Vector3, beyond: Vector3?) -> boolean` — move to an overshot point so the NPC does not brake at waypoints; false when every candidate point failed the floor check (the NPC has been stopped).
- API: `NPC:MoveForced(destination: Vector3)` — a plain `Humanoid:MoveTo` with no floor or lane check; the last resort of `Pursue`.
- API: `NPC:AdvanceWaypoint(waypoints: { PathWaypoint }, index: number) -> number` — skips waypoints already effectively reached.
- API: `NPC:WatchPatrolStall()` — starts the one-per-entry stall watchdog the shared `Patrol` state arms for itself. It samples the root every `NpcNavigationConfig.PatrolStallInterval`, resets whenever the NPC has travelled `PatrolStallDistance` studs from the last sample it reset on, and calls `Despawn` once `PatrolStallTime` seconds have passed without that happening — the bugged-in-the-scenery case, where an NPC is wedged on a chair or has nowhere it can path to. It exits the moment the machine leaves `Patrol`, so states that are meant to stand still (`Idle`, `Rest`, `Frozen`, `Stunned`, `RoomReaction`) are never counted; the inline `Wander` fallback inside `Patrol` still is, because it is also walking.
...
- API: `NPC:Pursue(getGoal: () -> Vector3?, arriveDistance: number?, hasArrived: (() -> boolean)?, canMoveDirectly: (() -> boolean)?) -> string` — the main chase loop; returns `"Reached"` or `"Lost"`. Every tick it tries, in order, until one actually issues a MoveTo: the floor-supported direct move (sighted target, straight line supported), the charge to the last-seen point, the next waypoint of the computed path (a waypoint the floor check refuses drops the route and backs off at once), a supported direct move when the goal has lane clearance, the hallway graph toward the goal (well-connected nodes, refreshed every 0.75s, skipping up to three nodes ahead that have lane clearance), and finally `MoveForced` straight at the overshot goal with no floor or lane check at all, so an enemy that can see you but has no computable route charges the straight line (holes included) instead of standing still.
...
- API: `NPC.SharedStates` — reusable state functions: `Attack`, `Idle`, `Wander`, `Patrol`, `Stunned`, `RoomReaction`, `Despawn`. `Patrol` only ever adopts and targets `WellConnected` graph nodes, so an enemy that ends up standing on a stranded pocket of floor walks back to the main network instead of pacing it forever. `Patrol(changeState, npc, onArrive)` takes an optional per-cycle hook called each time a route is walked to its end; returning a state name from it leaves Patrol for that state, which is how the Chaser drops into `Rest`.
...
- Requires: `ServerStorage.Classes.PropOddity`, `ServerStorage.Services.GazeService`, `ReplicatedStorage.Services.AudioService`, `ReplicatedStorage.Services.VfxService`; optional subclass hooks `OnFall`, `OnLoose`, `OnRestore`
...
### VoidPit.luau
Kill volume for a permanent pit. Bound to parts tagged `VoidPit`; every Heartbeat it kills any living player whose root part is inside the part's box, unless `VanishedService.Is` protects them, recording the part's `DeathCause` attribute (default `HallwayVoid`) as the cause. Used by the `KillVolume` under `Workspace.Maze15.HolePit`, which takes over the kill check the runtime `HallwayVoid` oddity ran.
- API: `VoidPit.new(instance: Instance) -> self` — starts the kill loop when the instance is a BasePart.
- API: `VoidPit:Destroy()` — disconnects `self.Connections`.
- Tags: listens `VoidPit` (through `ServerStorage\Modules\Tagger`)
- Requires: `ReplicatedStorage.Services.CharacterService`, `ReplicatedStorage.Services.VanishedService`, `ServerStorage.Services.DeathService`
...
### Enemies\Blind.luau
A deaf-to-sight, hearing-driven hunter: it registers an "ear" with `HearingService` and builds *Determination* from noises, which decays in silence and controls its speed tier (Investigate / Alert / Chase). Distinct behaviours are the flinch-and-turn "notice", coasting past a noise position after overshooting, playing the looping `Listen` animation override while standing at its search point, and only striking when highly certain. A landed strike deals `Damage` and then calms it down: the `Listen` override plays while it follows the victim at the slowed `Recover` pace for `RecoverTime`, after which it registers contact again and resumes `Pursue`.
- Pre-listening slowdown: investigation and pursuit share a time-based linear deceleration, capped by `OvershootDuration` (1 second). Travel is limited by obstacle clearance, `OvershootRange`, and the braking distance for the current speed. Braking happens after any travel to the search point; its final 0.6 seconds blend into the listening override, which continues without restarting once stopped. Normal completion fades the override out over ListenFadeOutTime (0.6 seconds), revealing locomotion smoothly. New-noise, attack and stun interruptions retain the short default transition; stationary listening duration is unchanged.
...
- Notes: overrides `NPC.new` and `BuildStateMachine`; writes the heartbeat `PursuitAttribute` on the model while pursuing or attacking; the Search state plays `Config.Animations.Listen` (preloaded at construction) through `NpcAnimator:PlayOverride` with `ListenFadeTime` (0.6 seconds) for the greater of `SearchTime` and the track's length, and Investigate/Pursue/Attack/Stunned each stop it on entry

### Enemies\CeilingDweller.luau
A Chaser that spawns on the ceiling and physically drops onto the floor before behaving normally: `Start` tweens the model down with collisions and humanoid states disabled, cues the victim's client camera ("Drop" then "Scream"), plays the `DwellerLanding` dust ring on the floor where it lands, and only then hands off to `NPC.Start`.
...
- Requires: `ServerStorage.Classes.Enemies.Chaser`, `TweenProxyService`, `VfxService`, `VfxConfig`
...
- Requires: `ServerStorage.Classes.EnemyBase`, `DeathService`, `EnemyDiscoveryService`, `RoomService`, `MathService.DistanceToSegment`, `VfxService`, `VfxConfig`
- Notes: extends EnemyBase; overrides `OnStart` / `OnDespawn`

### Enemies\Chaser.luau
The plain sight-based pursuer and the template most humanoid enemies build on: see a player, chase, attack, and fall back to Patrol. Between patrol routes it rolls `RestChance` to go and sleep: it walks to the wall of its current hallway (`RestWallInset` in from the corridor edge), turns to face the centre line so its back is against the wall, pauses the animator and loops the animation set's `Sleep` override for `RestTimeMin`-`RestTimeMax` seconds. It is deaf and blind while asleep - `Rest` is deliberately not in the `Perception` list - and wakes only when a player who is not `Crouching` comes within `RestWakeRadius` on the same corridor line, at which point it plays the `Wake` override for `RestWakeTime` and chases them; a stun still interrupts it, and both `Patrol` and `Stunned` clear the override so it can never be left asleep on its feet. Its own hits are `Recover` hits (it slows for `RecoverTime` and keeps coming); the state table also carries the shared `Retreat` state, which the Ceiling Dweller clone uses to run off and vanish after its single drop attack. It publishes `ChaseTargetUserId` for clients, reacts at safe-room doors when the target escapes into one, and randomly re-skins the model from `Config.Variants` (skin colour and hair) at construction.
- API: `Chaser.new(model: Model, config) -> self` — applies the visual variant first.
- API: `Chaser:BuildStateMachine() -> StateMachine` — Idle/Wander/Patrol/Rest/Chase/Attack/Search/RoomReaction/Stunned/Despawn with a `Perception` evaluator that is not active during `Rest`.
...
Follows a player from behind without ever being seen: it tails at `FollowDistance` while unobserved, for as long as it takes, and being looked at throws it into a Flee state that sprint-routes to a hide spot, rejecting any path that would carry it through a player's view cone. While stalking it matches the victim's measured ground speed and adds a catch-up boost the further it falls behind its follow point, so it never outpaces a player it is keeping up with but can always out-run one who sprints. The stalk ends only when it closes to `StrikeDistance` of the victim, at which point it Reveals (the strike deals `Damage` rather than killing, and once it lingers it goes straight into `Flee` with a silent despawn flagged, so it sprints for cover and disappears the moment it is out of sight) — snapping to `KillDistance` behind them (on its own side, or behind their facing when a wall is in the way) and re-snapping every frame until the strike so a sprinting victim cannot open a gap before their camera locks, seizing their camera, turning to face them, and killing. It also owns the shared Peek behaviour states, and its animation set carries the `PeekLeft`/`PeekRight` corner-peek clips those states pose it with.
...
Chases normally but freezes solid the instant any player observes it, and resumes the moment it is unobserved; after landing a hit it takes the shared `Retreat` state and runs off to vanish. Every non-Frozen state is wrapped in a `released` helper that clears the frozen attribute, and the perception evaluator runs at 0.05s so the freeze feels instantaneous.
...
A shared, non-class behaviour module: a set of state functions letting any NPC hide inside a corner mouth found by `PeekSpotService`, snap into the corner-peek pose while nobody is looking, hold it, and withdraw behind the corner with the `PeekLeft`/`PeekRight` clip, a rolled number of times (`PeekCountMin/Max`). The rig is anchored and posed by CFrame rather than walked: while lurking it stands upright at the spot's `HiddenRoot`; once the peek pose is outside every view cone it jumps to the spot's `Root` and holds the clip's first frame (the clip starts already peeking); when the player looks at it the clip plays from the start (its authored half-second stare, then the pull-back), when they walk up close or the hold expires it plays from `MotionStart` instead, and once the clip reaches `Settled` the root is moved to `HiddenRoot` and the clip is dropped, which lands exactly where the clip's root motion left the body. `SeenHoldTime` is extra wait before the stare. It follows the player between corners: whenever the spot has lost sight of them (`PeekSpotService:IsInSight` false for `FollowLostTime`) while lurking, peeking or resting, it enters `Follow`, waits `FollowDelay`, then teleports unseen to the next spot `PeekSpotService:Find` returns, retrying every `FollowRetryInterval` and ending as `Lost` after `FollowGiveUpTime`. Every posed peek counts towards the sequence; when it runs out it ends as `Done`, and it ends as `Seen` if the player closes in or looks directly at it. When the player catches it by looking (not by walking up), it stamps `PeekSpottedAt` on the model before the stare and `PeekWithdrawnAt` once it is hidden again (both `workspace:GetServerTimeNow()`, names from `VfxConfig.Stalker`), which `StalkerGlowService` turns into the eye flare.
...
- API: `Peek.GetFindOptions(config) -> PeekSpotService.FindOptions` — builds find options from the enemy config, reading rig scale and root height off the `ReplicatedStorage.Enemies` template named by `Model`.
- API: `Peek.Enter(npc: any)` — anchors the root, stops movement, rolls `PeeksLeft`, preloads both peek clips.
- API: `Peek.Exit(npc: any)` — drops any peek pose, stands the root upright at its real standing height above the floor, unanchors, restores the humanoid state machine in `Running`, and restores walk speed.
- Requires: `PeekSpotService`, `EnemyObservationService`, `MathService`, `Configs.PeekPoseConfig`, `Configs.VfxConfig`; the host NPC must implement `OnPeekFinished(reason: string, player: Player?)`, carry `PeekLeft`/`PeekRight` in its animation set, and supply config keys `PeekCountMin/Max`, `MinPeekDistance`, `MaxPeekDistance`, `PeekFogFraction`, `PeekExposure`, `RearArc`, `ViewCone`, `PeekWaitTime`, `HoldTimeMin/Max`, `RestTimeMin/Max`, `SeenHoldTime`, `FollowLostTime`, `FollowDelay`, `FollowRetryInterval`, `FollowGiveUpTime`
### Oddity.luau
...
- API: `HallwayCrush:_paperEnds(part: BasePart)` — gives a moving wallpaper piece a Texture on any bare end face, cloned from a broad face so the transparency matches the surrounding paper, and records it in `self.Papers` for deletion on restore; a no-op on the trim layers, which carry no Texture to clone
...
- API: `PaintingDweller:OnStart() -> boolean` — builds the rig, hides decals, plays the pop tween with the one-shot `StartAnimation` handing off to the looping `ThrashAnimation`, plays the `PaintingBurst` canvas-scrap and dust burst out of the canvas face, plays `PopSound` once and loops `ScreamSound` and `RustleSound` on the rig root, starts the attack poll
...
Extends `HallwayOddity`. Fades every eligible world part that sits mostly inside a hallway box to near-invisible, so the corridor's geometry seems to vanish; skips character parts and anything tagged `TransparencyIgnore`, and uses a module-level refcount so overlapping instances restore correctly. Disabled with `Enabled = false` in `MapOddityConfig`: it no longer spawns ambiently or from `/mapoddity`, and a permanent copy is baked into the map as the Invisible Hallway point of interest (see `Documentation\Workspace\POIHallways.md`).
...
Server half of the Gravity Warper: on activation it asks the server `GravityWarpService` whether the holder can warp (alive, not already warping, ceiling above), consumes one, then starts the warp through that same service with the tool's `Duration`. Player and duration are captured before `Consume`, because consuming the last charge destroys the Tool synchronously. The `GravityWarping` attribute, the `IgnoreExceptEye` tag, the `GravityWarp/Warp` and `GravityWarp/Finished` remotes and the death/fallback clears all live in `Services\GravityWarpService`, shared with the Sisters eye-contact catch.
- API: `GravityWarper.new(tool: Tool)`
- Requires: `Classes\ServerTool`, `Services\GravityWarpService`
