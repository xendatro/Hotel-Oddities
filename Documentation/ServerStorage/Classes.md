...
- API: `HallwayCrush:_paperEnds(part: BasePart)` — gives a moving wallpaper piece a Texture on any bare end face and records it in `self.Papers` for deletion on restore; a no-op on the trim layers, which carry no Texture to clone
