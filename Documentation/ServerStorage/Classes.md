...
### Enemies\Chaser.luau
The plain sight-based pursuer and the template most humanoid enemies build on: see a player, chase, attack, and fall back to Patrol. Its own hits are `Recover` hits (it slows for `RecoverTime` and keeps coming); the state table also carries the shared `Retreat` state, which the Ceiling Dweller clone uses to run off and vanish after its single drop attack. It publishes `ChaseTargetUserId` for clients, reacts at safe-room doors when the target escapes into one, and randomly re-skins the model from `Config.Variants` (skin colour and hair) at construction.
