...
ExitElevator.PrimaryPart is its Hitbox. The Hitbox PivotOffset is set to the model pivot it had before, so GetPivot still returns (-592.883, -13.717, 530.798) and ElevatorDoorService distances are unchanged. EscapeMusicService finds the exit through CharacterService.GetStreamedPivot, which returns nil for a model with no PrimaryPart, so the escape theme never started until this was set. The build tool now sets it the same way. StartElevator and the lobby elevator still have no PrimaryPart.
