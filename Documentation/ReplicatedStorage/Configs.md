...
Client gating and placement for the Chaos warning ambience: how often to re-evaluate, which bus/folder/sting to play, and how far outside a warned hallway a listener in an adjacent hallway or an adjacent room still counts. `FullVolumeDistance`, `SilentDistance` and `StingDistance` are measured to **Chaos itself**, not to the hallway, so they are sized against its `WalkSpeed` of 70 rather than against a room: 380 studs is about 5.4 seconds of lead, 70 is about one second, and 260 gives the sting roughly 3.7 seconds to be worth reacting to. `StingDistance` is deliberately tighter than `SilentDistance`: the bed is allowed to bleed in faintly from a long way off, but the full-volume 2D `IncomingSound` is not. `SuppressGain` is the bed level at which the ordinary ambience playlist is worth ducking.
...
- API: data table — `Tag`, `Padding`, `FloorTolerance`, `RetryDelay`, `CastShadow`, `BendLocalLook`, `ReflectEnemies`, `StripClasses`
