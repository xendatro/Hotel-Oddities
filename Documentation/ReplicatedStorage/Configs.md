...
### AmbienceConfig.luau
Distance-based volume falloff and fade timing for ambient sound emitters, plus the POI cross-fade and unsettling pitch, distortion and tremolo tuning used while inside a POI. `LocationAmbience` sets the local POI fade distance, fade time and volume; `ExtraAmbience` sets the random one-shot interval bounds.
- API: data table — `SilentDistance`, `FullVolumeDistance`, `FadeTime`, `UpdateInterval`, `POI` (`Volume`, `FadeTime`, `Pitch`, `DistortionLevel`, `TremoloDepth`, `TremoloFrequency`), `LocationAmbience` (`FadeDistance`, `FadeTime`, `Volume`), `ExtraAmbience` (`MinInterval`, `MaxInterval`)
...
Per-effect tuning for prop-based oddities — falling lanterns, falling paintings, the painting dweller (whose `Damage` is what each lunge takes off the player), and the scurrying rat — covering arming, approach detection, candidate selection, and either repair rules (the fixture effects) or crossing-site sampling and rat motion (`RatScurry`). `LanternFall` and `PaintingFall` use `ObjectFalling` as their `ImpactSound` on floor contact, and play their `ImpactVfx` template (`LanternShatter`, `PaintingThud`) at the same moment through `VfxService`. `PaintingDweller` carries `StartAnimation` (one-shot burst-out), `ThrashAnimation` (loop that follows it), `AttackAnimation`, `HoleImage`, `RootDrop`, the studs the rig hangs below the canvas centre, and the `ReplicatedStorage.Sounds` template names `PopSound` (played once on the pop), `ScreamSound` and `RustleSound` (both looped until it retreats); its fixture debug highlight is disabled.
...
### VfxConfig.luau
Settings for `VfxService` and the effects built on it. `Folder` is where templates live (`ReplicatedStorage.Effects`), `Remote` is the `Vfx/Play` remote, `HolderFolder` is the client folder live effects go in, `MaxDistance` (220) is the default camera cull distance, and `CleanupPadding` is the extra time before a finished holder is destroyed. `Templates` names every template a script plays by name: `ChaosCrash`, `VentDust`, `DwellerLanding`, `PaintingBurst`, `CrushDust`, `CrushSeal`, `FlashlightDust`, `StalkerWisp`. The fixture impacts `LanternShatter` and `PaintingThud` are named in `PropOddityConfig`. Per-effect sections: `Chaos.WallProbe` (how far past the route end to look for the wall), `Vent.DustDuration`, `Crush` (ceiling search, fallback height, near-wall probe offset, ceiling inset, the close fraction that counts as sealed, the shortest interval that gets dust), `Flashlight.Distance` (studs down the beam), and `Stalker` (the two attribute names, eye and head names, glow template path, glow scale and offset, flare curve and heat, withdraw fade, backstop time, wisp lift and cull distance). Particle look is tuned on the template instances, not here.
- API: data table
