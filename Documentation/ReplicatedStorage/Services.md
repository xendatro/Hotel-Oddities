...
Viewmodel clones freeze the equipped tool's captured geometry after removing joints, cache each tool's unscaled bounds for stable rebuilds, use the clone's Handle as the pivot reference, and reset sway and bob state when the equipped tool changes so repeated equip cycles do not move its baseline offset.
