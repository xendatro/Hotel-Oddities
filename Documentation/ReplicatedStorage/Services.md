...
Owns the main menu page group: enables/disables the Index, Rooms, Shop, Gems, Items, Inventory, KitInventory, KitsShop, RollGui, Map and Gallery ScreenGuis through an xenterface controller, blurs and pulls back the camera FOV while a page is open, and manages mouse unlocking (including a Q toggle when no page is open, even while the full-screen mouse blocker has processed the input). Also wires hover/press motion onto tagged side buttons and close buttons using named motion presets; close buttons that sit directly on a page root animate themselves rather than scaling the full-page frame. Every tagged side button whose `PageId` appears in `SideButtonConfig` gets a runtime `SideButtonLabel` TextLabel across its lower edge naming the page, with the keybind appended as `Items [I]` only while `UserInputService.KeyboardEnabled` is true, refreshed whenever that changes, so touch players see the name alone. Those same keys toggle their page open and shut (Items I, Kits K, Shop G, Inventory E); a key that is also an interaction key is left to the interaction whenever something is under the crosshair, so E opens the bag only when nothing is there to use. The Map page is rejected unless the player owns the Map gamepass.
