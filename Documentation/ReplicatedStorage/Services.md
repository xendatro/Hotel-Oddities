...
A region is only audible when the player is **inside** it, standing in another hallway whose closest approach to the region's box is within `AdjacentHallwayRange` **and not behind a wall**, or in a room whose doorway threshold sits within `RoomDoorRange` of the box. The adjacent-hallway ray is cast with an **Include** filter of every `Walls` folder in the workspace rather than an exclude list, so only real structure occludes: furniture, glass, fixtures and other players never silence a listener standing at an open corner, but the parallel corridor one wall over — which used to hear the bed with every red light on the far side of that wall — no longer qualifies. An empty filter (no maze loaded) fails open.

**Volume is Roblox's, not this service's.** The emitters keep the `DistanceAttenuationMode` and `DistanceAttenuationBounds` they are authored with (`Inverse` over `10 → 35` at the time of writing) and the service never calls `SetDistanceAttenuation`. Fades are applied through an `AudioFader` spliced between each cloned `AudioPlayer` and its emitter — the player's own `Volume` is owned by `AudioService`'s bus binding and must not be written here. That split is what lets "audible slightly outside the hallway, silent well away from it" be a property of the rolloff curve rather than of a hand-written gain function.

**The source sits on the warned hallway's centre line at the point nearest the listener, and moves as they move.** Walking up and down the corridor cannot outrun it; only leaving the corridor does, because then the lateral distance grows and the rolloff takes over. Chaos's own position is deliberately *not* the source — an emitter placed on Chaos and a rolloff opened wide enough to still be audible 1050 studs up-corridor is exactly the bug where the whole map could hear him. His position is used for one thing only: latching the pass that cuts the sound. The room case pins the source at the doorway projection instead, since that point is fixed in the world.

Timing is per-listener, not per-region, and it is the same wave the lights ride: the payload carries server-time arrivals for the span's two ends, and the service lerps them to the listener's own position along the span. Nothing is audible until Chaos is due to reach *them* within that region's `WarningTime` — the exact instant the lamps beside them turn red and start swinging — and it cuts the moment he passes. A warned corridor is therefore not uniformly loud: the red front sweeps down it and the sound arrives with the front, not with the broadcast. When several regions qualify the one Chaos reaches soonest wins.

The `ChaosIncoming` sting fires **once per Chaos run**, positioned on the same centre-line point as the bed so its own rolloff decides who hears it. Keying it on the run rather than the region matters because one route raises a separate `ChaosWarning` per span along it: walking into the next span's range, or Chaos crossing into a new span as it passes, would otherwise each re-trigger the cue. The run id is forgotten once that run's last region stops. It is armed only by a *predicted-arrival* candidate — Chaos still up-corridor and due within `WarningTime` — and takes the soonest such candidate, never a pass-tracked one, because the pass branch carries no lead-time gate and would fire an "incoming" for something already going past. It is played non-looping through `Play3DSound`, so the clone tears itself down when it ends. Its template's `PlaybackRegion` starts 0.2s in, trimming the head silence so the cue lands on the frame the red does.

Do **not** reintroduce a `DistanceAttenuationMode.Custom` override here. An earlier version set every cloned emitter to `Custom` over `[0, AttenuationRange]` with a flat curve it wrote itself, which discards the authored rolloff completely and makes the bed's reach a number in this file rather than a property of the sound. Two bugs came out of that, and both are worth remembering: writing a custom curve *without* also setting the mode is silently ignored (Roblox only reads the curve in `Custom` mode), and setting the mode but computing the distance from the listener's own position along the span cancels the along-corridor term, so an entire 960-stud corridor sits at full volume. The anchor lerps between targets and snaps only on jumps over `AnchorSnapDistance`; the `AudioFader` ramps with separate fade-in/out speeds and snaps to its target within `GainSnap`, so silence actually reaches zero and tears down. Cut-off is driven by the **real Chaos model**, not the predicted arrival, which is what makes it agree with the lights. Whenever a tracked Chaos is inside the warned region, `ChaosTrackerService.AlongTo` gives its signed distance to the reference point (the listener, or their doorway for the room case): the fader ducks over the last `PassFadeDistance` studs of that approach. Note that this attributes any tagged Chaos inside the box to *that region's* run, which is only safe because `MaxAlive` is 1.
...
While the listener is **inside** a warned region the service holds `AmbienceService:Suppress`, muting the game's ordinary ambience playlist, and releases it on every exit path — including when the last warning stops and when the voices tear down. Inside-the-region rather than "any audible region" is the condition because a listener merely adjacent to the corridor hears the bed at the quiet tail of its rolloff, and cutting the ordinary ambience for that replaces it with a hole rather than with a bed anyone can hear.
