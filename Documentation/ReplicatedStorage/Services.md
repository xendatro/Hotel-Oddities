...
A region is only audible when the player is **inside** it, standing in another hallway whose closest approach to the region's box is within `AdjacentHallwayRange` **and not behind a wall**, or in a room whose doorway threshold sits within `RoomDoorRange` of the box. In the last two cases the anchor is still placed on the warned hallway's own centre line — at the listener's height, or at the doorway's projection for the room case — so direction and distance read as coming out of that hallway rather than from the region's midpoint. The adjacent-hallway ray is cast with an **Include** filter of every `Walls` folder in the workspace rather than an exclude list, so only real structure occludes: furniture, glass, fixtures and other players never silence a listener standing at an open corner, but the parallel corridor one wall over — which used to hear the bed at near-full gain with every red light on the far side of that wall — no longer qualifies. An empty filter (no maze loaded) fails open.

The distance that drives the gain is the listener's distance to **where Chaos is**, not to the hallway. Each tick the service resolves Chaos's position along the region: a live tagged Chaos inside the box wins, otherwise the arrival lerp is inverted to predict it, pinned at the entry end before it has entered and the exit end after. While it has not entered yet, the time still to run is converted to studs — the region's own length over its arrival window gives the speed, no payload field needed — and added to the distance, so a listener standing at the mouth of the corridor does not get the full bed fifteen seconds early. This is what was wrong before: the source was taken at the *listener's* own position along the span, which cancels the along-corridor term entirely, so every point in a warned corridor measured a few studs from the centre line and sat at gain 1.0 for the whole `WarningTime` — 15 seconds, or about 1050 studs at Chaos's `WalkSpeed` of 70. The distances in `ChaosWarningConfig` are therefore sized against that speed rather than against a room: silent past `SilentDistance` (380, ~5.4s of lead), full by `FullVolumeDistance` (70, ~1s), and the curve is `(1 - alpha)^2` so it stays a faint bed until Chaos is genuinely close instead of `1 - alpha^2`'s near-full plateau.

Timing is per-listener, not per-region: the payload carries server-time arrivals for the span's two ends, and the service lerps them to the listener's position along the span, staying silent unless Chaos reaches *them* within that region's `WarningTime` and cutting out the moment it passes. That is what keeps the sound in step with the lights going red instead of firing when a far end of a long hallway was warned. When several regions qualify the one Chaos reaches soonest wins. The `ChaosIncoming` sting fires once per Chaos **run** and plays **2D**, so it cannot be lost to positioning or to the anchor being torn down under it. Keying it on the run rather than the region matters because one route raises a separate `ChaosWarning` per span along it: walking into the next span's range, or Chaos crossing into a new span as it passes, would otherwise each re-trigger the cue. The run id is forgotten once that run's last region stops.

Because it is 2D it always plays at full volume, so it is gated more tightly than the bed and **independently of which region wins the bed**. It is armed only by a *predicted-arrival* candidate — Chaos is still up-corridor and due within `WarningTime` — that is also within `StingDistance` (260, ~3.7s of lead), tracked as its own nearest-approach candidate during the scan. Neither condition can be dropped. Gating it on the bed's winner instead would let a pass-tracked region steal the decision: a region whose gain merely cleared zero is audible out to `SilentDistance`, and the pass branch carries no lead-time gate at all, so Chaos sprinting down a parallel corridor used to fire a full-volume "incoming" for something already going past. The bed would then duck to near-nothing and hard-cut as the pass latched a moment later, leaving a full sting attached to a half-second of near-silence and no red lights anywhere near the listener. Burning the run's one sting there also meant no cue at all when Chaos genuinely came down their hallway later in the same run.

Playback is driven explicitly rather than left to Roblox's rolloff, because an emitter parented to a point recomputed from the listener every tick sat almost on top of them, and at near-zero distance the pan swung wildly with camera rotation. Each cloned ambience emitter is switched to `DistanceAttenuationMode.Custom` with bounds of `[0, AttenuationRange]` and then given a flat curve this service writes itself, so gain comes from its own curve over the listener-to-Chaos distance and is completely decoupled from where the anchor sits. **Setting the mode is not optional** — the templates are authored as `Inverse` over `[10, 60]`, and Roblox ignores a custom curve entirely unless the mode is `Custom`, so without it every gain write here is silently discarded and the bed plays at whatever the raw rolloff and the camera-parented listener decide. That frees the anchor to be placed purely for *direction*: on the warned hallway's centre line, at Chaos's own resolved position along the span, pushed no closer than `MinSourceDistance` from the listener along the approach direction (it may extend past the span end, which is fine). The result is a source that reads as "up that corridor, where it is coming from", travelling down the corridor as Chaos does, and never collapsing onto the listener. The room case keeps the anchor at the doorway instead, since that point is fixed in the world and does not chase the listener. The anchor lerps between targets and snaps only on jumps over `AnchorSnapDistance`; gains ramp with separate fade-in/out speeds and the emitter write is throttled independently of the ramp, so the ramp always lands exactly on its target and silence actually reaches zero and tears down. Cut-off is driven by the **real Chaos model**, not the predicted arrival, which is what makes it agree with the lights. Whenever a tracked Chaos is inside the warned region, `ChaosTrackerService.AlongTo` gives its signed distance to the reference point (the listener, or their doorway for the room case): the gain ducks over the last `PassFadeDistance` studs of that approach. The predicted arrival is still what opens the warning, since Chaos has not spawned yet 15 seconds out; real geometry takes over as soon as it enters the hallway.

Whether Chaos has gone past is a **sticky per-run latch**, not a per-tick test, mirroring the `Engaged`/`Passed` pair in `ChaosLightService`. A run engages once a mover comes within `PassEngageRange` of the reference and latches passed when `along` goes negative; every region of a latched run is then skipped until the run's last region stops. Without the latch the check is stateless, and since Chaos clears the region's halo about a third of a second after passing at 70 studs/s, the observation would evaporate and the region would silently fall back to the server's optimistic schedule and re-open the bed behind it. The latch transition also hard-cuts the voices to zero rather than ramping, so there is no tail.

While the bed's target gain is at or above `SuppressGain` the service holds `AmbienceService:Suppress`, muting the game's ordinary ambience playlist, and releases it on every exit path — including when the last warning stops and when the voices tear down. It is a gain threshold rather than "is any region audible at all" because the two are not the same near `SilentDistance`: a region can win with a gain of a few hundredths, and cutting the ordinary ambience for that replaces it with a hole rather than with a bed anyone can hear.
...
- Requires: `Configs.ChaosWarningConfig`, `Services.HallwaysService`, `AudioService`, `AmbienceService:Suppress`, `ChaosTrackerService`, `CharacterService`, `CommunicationService`
